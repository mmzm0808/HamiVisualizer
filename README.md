...
- “文件 → 打开最近模型”从 `~/.hvisual/models/` 读取最近记录；损坏或不存在的记录会被
  安全跳过，可在菜单中清空记录。同名模型会在菜单项后显示父目录，悬停可查看完整路径，
  避免跨实验目录时选错文件。
...
- “系数：全部显示”把跃迁系数输入框集中在右侧导轨。同一条物理线的正向、反向或重复、
  且属于同一参数族的表格行只显示一个输入框，tooltip 会列出合并行号；如果同一几何线
  同时承载独立参数（例如 `t` 与 `t2`），会分别保留输入框并在关系标签中标明参数名，
  不会把一个参数的修改误套到另一个参数。密集模型默认收起引线，悬停跃迁或聚焦输入框
  时只显示当前项的斜向+横向导引，避免满屏交叉线。
- 编辑工具栏提供“吸附”“网格点”“间距”“恢复编辑前构型”“撤销/重做”；网格节点与
  实际吸附坐标一致，关闭“网格点”只隐藏背景点，不会改变吸附规则；间距数值越小网格越密。默认最多保留 10 步，可在偏好
...
兼容说明：`phase_mode=directional` 是 version=1 旧模型中保留的方向相位预留值，当前不可计算。
旧文件仍可导入检查并原样保存；导入后请将跃迁表中的该行改为 `none`（实跃迁）或
`phase`（固定相位）后再计算。新建跃迁只使用当前支持的 `none`/`phase` 模式。
...
数学字体排版，`x` 位于 `k` 的下标位置。零元素为了降噪不在格内画文字，但仍可通过悬停或
点击查看精确值；超过 64 阶后切换为高性能栅格图时，也保留按单元格的悬停定位。提示会同时给出
四边逻辑标签和从 1 开始的 `H[row,col]` 坐标。
...
页只在已构造有限矩阵后生效，按能量选择最近本征态，并显示满足
`sum_i |psi_i|^2 = 1` 的真实概率密度和边界权重。点击任意波函数格点可固定检查从 1 开始的
格点编号、坐标、边界/体内属性与该点概率；再次选择本征态时会沿用当前格点并更新概率。
...
“智能识别标签”并不是从矩阵元的小数值猜参数名。数值建阵时会同步保留稀疏的来源表达式，
因此 SSH 的 `t1/t2`、Haldane 的 `t2/m/phi` 以及自定义参数即使当前数值相同，也仍按真实
来源显示为 `t_{1}`、`t_{2}` 等数学下标；来源缺失或与数值回算不一致时，该矩阵元会安全
退回数值显示。切换“符号模式”仍可查看完整的全符号 Hamiltonian。
